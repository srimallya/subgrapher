...
function buildGuardedScript(userCodeRaw) {
  ...
  const usesPygame = /\bpygame\b/.test(userCode);
  ...
  return [
    ...
    'import builtins',
    '',
    '# Provide a minimal requests-compatible fallback when requests is unavailable.',
    'def _install_requests_fallback():',
    '    try:',
    ...
    '    except Exception:',
    '        return',
    '    class _LuminoResponse:',
    '        def __init__(self, url, status_code, headers, content):',
    '            self.url = str(url or "")',
    '            self.status_code = int(status_code or 0)',
    '            self.headers = headers or {}',
    '            self.content = content if isinstance(content, (bytes, bytearray)) else bytes(content or b"")',
    '            self.ok = 200 <= self.status_code < 400',
    '        @property',
    '        def text(self):',
    '            enc = str(self.headers.get("content-type", ""))',
    '            charset = "utf-8"',
    '            marker = "charset="',
    '            if marker in enc.lower():',
    '                try:',
    '                    charset = enc.lower().split(marker, 1)[1].split(";", 1)[0].strip() or "utf-8"',
    '                except Exception:',
    '                    charset = "utf-8"',
    '            try:',
    ...
    '    def _request(method, url, params=None, data=None, json=None, headers=None, timeout=30, **kwargs):',
    '        target = str(url or "")',
    '        if params and isinstance(params, dict):',
    '            query = _urllib_parse.urlencode(params, doseq=True)',
    '            target = f"{target}&{query}" if "?" in target else f"{target}?{query}"',
    ...
    '        req = _urllib_request.Request(target, data=body, method=str(method or "GET").upper())',
    '        for key, value in req_headers.items():',
    '            req.add_header(str(key), str(value))',
    '        try:',
    '            with _urllib_request.urlopen(req, timeout=float(timeout or 30)) as resp:',
    '                content = resp.read()',
    '                return _LuminoResponse(resp.geturl(), int(getattr(resp, "status", 200) or 200), dict(resp.headers or {}), content)',
    '        except _urllib_error.HTTPError as err:',
    '            content = b""',
    '            try:',
    '                content = err.read()',
    '            except Exception:',
    '                content = b""',
    '            return _LuminoResponse(getattr(err, "url", target), int(getattr(err, "code", 500) or 500), dict(getattr(err, "headers", {}) or {}), content)',
    '    _module = _types.ModuleType("requests")',
    '    _module.request = _request',
    '    _module.get = lambda url, **kwargs: _request("GET", url, **kwargs)',
    '    _module.post = lambda url, **kwargs: _request("POST", url, **kwargs)',
    '    _module.put = lambda url, **kwargs: _request("PUT", url, **kwargs)',
    '    _module.patch = lambda url, **kwargs: _request("PATCH", url, **kwargs)',
    '    _module.delete = lambda url, **kwargs: _request("DELETE", url, **kwargs)',
    '    _module.head = lambda url, **kwargs: _request("HEAD", url, **kwargs)',
    '    _module.Response = _LuminoResponse',
    '    sys.modules["requests"] = _module',
    '',
    '_install_requests_fallback()',
    '',
    'os.environ.setdefault("SDL_VIDEODRIVER", "dummy")',
    'os.environ.setdefault("SDL_AUDIODRIVER", "dummy")',
    `_ENABLE_PYGAME_RUNTIME = ${usesPygame ? 'True' : 'False'}`,
    '',
    ...matplotlibSetup,
    ...
    '        pass',
    '    try:',
    '        if _ENABLE_PYGAME_RUNTIME and "pygame" in sys.modules:',
    '            import pygame',
    '            if pygame.get_init():',
    '                pygame.quit()',
    '    except Exception:',
